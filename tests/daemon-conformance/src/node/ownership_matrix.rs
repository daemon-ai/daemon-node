...
fn classify(req: &ApiRequest) -> Coverage {
    ...
    match req {
        ...
        // Vhc training (spec §10.4): node-wide operator control, not per-owner session state.
        VhcRunList
        | VhcRunDetail { .. }
        | VhcJoin { .. }
        | VhcLeave { .. }
        ...
        // Saved presences are node-wide shared config, not per-owner session state: the

...
use crate::{
    ...
    StoredCronRun, StoredCronSuggestion, TraceEntry, TraceSegment, TransportPref,
    CRON_RUN_RETENTION,
};
...
static MIGRATIONS: LazyLock<Migrations<'static>> = LazyLock::new(|| {
    Migrations::new(vec![
        ...
        // Its own queue table (mirroring `wake_outbox`), not a `session_meta` column — it is a
        // FIFO, not a property.
        M::up(
        ...
        // M13 (custom providers — generalized Daemon Cloud): the durable half of the provider
...
mod tests {
    ...
    /// `credential_labels` account-management tables, and the `custom_providers` table).
    #[test]

...
//! Adapter ops<->behavior conformance: the [`assert_ops_match_behavior`] invariant against the real `LineAdapter`,
...
use daemon_line::{LineAdapter, LineConfig};
...
async fn ops_match_behavior() {
    let adapter = LineAdapter::new(Arc::new(NoProvisioning), LineConfig::default());
    let proto: Arc<dyn MessagingProtocol> =
        adapter.messaging().expect("line is a messaging protocol");

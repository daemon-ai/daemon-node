...
fn seeded(mode: RecallMode) -> Engine {
    let cfg = MnemosyneConfig { recall_mode: mode, ..MnemosyneConfig::default() };
    let e = Engine::open_in_memory(cfg).unwrap();
    let rows: &[(&str, [f32; 3])] = &[
        ("the database password rotation policy runs monthly", [1.0, 0.0, 0.0]),
        ("Acme Corporation signed the contract last week", [0.0, 1.0, 0.0]),
    ...
    for (content, vec) in rows {
        e.remember_with_vector(content, &RememberArgs::default(), Some(vec), "mock").unwrap();
    }
    e
}
...
fn base_recall_is_ranked_but_does_not_expand_synonyms() {
    ...
    assert!(!hits.is_empty(), "base recall should surface the lexical match");
    assert!(hits[0].content.contains("password"), "top hit: {}", hits[0].content);
...
fn enhanced_recall_expands_synonyms_and_is_stable() {
    ...
    assert!(!hits.is_empty(), "enhanced recall should expand `db` -> database");
    assert!(hits[0].content.contains("password"), "top hit: {}", hits[0].content);

    // Repeating the query (served from the semantic cache) stays consistent.
    let again = e.recall("db", 5).unwrap();
    assert_eq!(again[0].content, hits[0].content, "cached recall must be stable");
}
...
fn polyphonic_recall_fuses_voices_over_seeded_bank() {
    ...
    let hits = e.recall_with_vector("Acme", 5, Some(&[0.0, 1.0, 0.0])).unwrap();
    assert!(

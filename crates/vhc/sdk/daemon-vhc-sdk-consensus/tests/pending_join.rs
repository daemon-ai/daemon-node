...
use daemon_vhc_sdk_consensus::coordinator::{tick, Input, Phase};
...
fn complete_round(
    mut state: daemon_vhc_sdk_consensus::coordinator::CoordinatorState,
    ks: &[daemon_vhc_proto::SigningKey],
    coord: &daemon_vhc_proto::SigningKey,
    round: u64,
    seed: u8,
) -> daemon_vhc_sdk_consensus::coordinator::CoordinatorState {
    for k in ks {
...
fn join_after_warmup_transition_is_staged_pending() {
    ...
    assert!(
        out.iter().any(|o| matches!(
            o,
            daemon_vhc_sdk_consensus::coordinator::Output::Note(
                daemon_vhc_sdk_consensus::coordinator::Notice::Admitted(_)
            )
        )),

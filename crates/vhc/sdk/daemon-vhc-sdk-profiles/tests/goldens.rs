...
// (0xDAE0_7E57 / 0x1234 / 0xABCD / 0x5150), captured as literals from the pre-deletion commit by
// re-running each retired A/B scenario's exact drive — for the error-feedback scenario that
// includes the retired implementation's own per-round ingest + round-base snapshot mutating the
// reference masters between rounds, so `EF_THETA1` is the θ the oracle really trained after its
// round-0 ingest. The pinned outputs are, per test: `EF_MASTER0`/`EF_MASTER1` — the RETIRED
// implementation's own post-ingest masters (the oracle outputs, captured directly);
// `SDK1_WANT`/`SDK4_WANT_CLIPPED`/`DILOCO_WANT_NESTEROV`/`DEMO_WANT` — the literals the retired
// A/B suite already pinned (bit-equal to both implementations when the A/B last ran);
// `DILOCO_PLAIN_MASTER` — this crate's output over the pinned inputs, recorded while the live
// A/B still held. The end-to-end drift oracle for the profile math is the trainer-goldens
// bundle (`daemon-vhc-host/tests/trainer_goldens.rs`), which pins digests + committed payloads
// through the production trainer guest.
//
...
const EF_MASTER0: &[u32] = &[
    1033547149, 1035905016, 3166582922, 3179708986, 1032789417, 1042697249, 1047616210, 1037443080,
    1034583887, 1022732346, 3172849934, 3182726122, 3193082520, 3174009292, 1024065251, 1033504078,
    1023940144, 3192690580, 1024670989, 1017043157, 1037873341, 3182951129, 1000092354, 3184372672,
    1033046889, 3186178491, 3179967907, 1039907904, 1033900537, 1041321735, 1041618040, 3167734888,
    3173339091, 1043485881, 1039702757, 1026832117, 3188086045, 1035097131, 1019562548, 1011522219,
    1039729050, 1036564907, 3170391000, 1034899158, 3157006803, 3183209039, 1024173708, 3172584317,
    1045448963, 1027019828, 3160831576, 1042388084, 1038197015, 3169967192, 1032671187, 1042616698,
    1029413506, 1036349793, 1042426432, 3160522523, 1023866034, 3154456784, 1024394021, 3170296224,
];
const EF_THETA1: &[u32] = &[
    1051851838, 1052682015, 1048844690, 1046292935, 1051920623, 1054930987, 1057097153, 1053056692,
    1052359525, 1050606149, 1048096681, 1044797194, 1033472916, 1047639293, 1050728421, 1051841475,
    1050713033, 1034250811, 1050802909, 1050255979, 1053161344, 1044685590, 1049772178, 1043980328,
    1051731427, 1043084017, 1046164636, 1053655145, 1051936844, 1054270759, 1054413403, 1048685605,
    1047975661, 1055306993, 1053605439, 1051068515, 1041783340, 1052224325, 1050411093, 1050023662,
    1053611811, 1052842873, 1048468522, 1052176800, 1049266337, 1044557656, 1050741759, 1048162391,
    1056232502, 1051091573, 1049148261, 1054783047, 1053240020, 1048520816, 1051891673, 1054588801,
    1051385441, 1052790453, 1054499438, 1049157803, 1050703918, 1049254573, 1050768852, 1048615195,
];
const EF_MASTER1: &[u32] = &[
    1033547149, 1035905016, 1054489719, 1048903538, 1032789417, 1042697249, 1047616210, 1037443080,
    1034583887, 1022732346, 1053975774, 1047722507, 3193082520, 3174009292, 1024065251, 1033504078,
    1023940144, 3192690580, 1024670989, 1017043157, 1037873341, 1052270922, 1000092354, 1046899232,
    1033046889, 1045996322, 1053016727, 1039907904, 1033900537, 1041321735, 1041618040, 3167734888,
    1049753094, 1043485881, 1039702757, 1026832117, 3188086045, 1035097131, 1019562548, 1011522219,
    1039729050, 1036564907, 3170391000, 1034899158, 1054916401, 1047481048, 1024173708, 1054008976,
    1045448963, 1027019828, 1050618957, 1042388084, 1038197015, 3169967192, 1032671187, 1042616698,
    1029413506, 1036349793, 1042426432, 1054790151, 1056358038, 3154456784, 1024394021, 1050079718,
];
...
fn sparse_loco_error_feedback_carries_across_rounds() {
    ...
    assert_eq!(
        bits(&master0),
        EF_MASTER0,
        "ef round-0 pinned golden (the retired oracle's post-ingest master)"
    );

    // Round 1 trains from the pinned trajectory (the oracle's θ AFTER its round-0 ingest +
    // round-base snapshot mutated the reference masters); the round-0 post-ingest master is the
    // next round base, and the profile's error-feedback residual carries between the rounds (the
    // pinned output is only reproducible when it does).
    ...
    assert_eq!(
        bits(&master1),
        EF_MASTER1,
        "ef-carry pinned golden (the retired oracle's round-1 post-ingest master)"
    );
}

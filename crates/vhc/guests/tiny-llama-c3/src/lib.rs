...
//! staged bytes and are **blake3-verified in-guest** at `Staged::mint`.
//!
...
use daemon_vhc_sdk_rounds::{
    BarrierRound, PayloadSource, RoundCfg, RoundExperiment, Staged, StepCtx as RoundStepCtx,
};
...
impl RoundExperiment<Vec<u8>> for C3Round {
    ...
    fn ingest(&mut self, round: u64, staged: &Staged<Vec<u8>>) -> [u8; 16] {
        let mut core = self.core.borrow_mut();
...
/// `Staged::mint` re-verifies each against the record-listed blake3 in-guest.
struct PayloadMap {

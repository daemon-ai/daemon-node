...
//! **leadership term**, but never judges authority. This manager owns the CLAIMANT half:
//!
//! - **term-bid derivation** from the slot's current state AND the node's own persisted verified
//!   floor (a registry serving a stale/low floor never lowers the bid — registry responses drive
//!   retry logic, never trusted state);
//! - **lease authorship**: provision the coordinator per-run identity (mint key + certificate
//!   under the base identity, [`crate::credentials`]/[`daemon_vhc_session::provisioning`]) at a
//!   **counter-minted execution incarnation** — the incarnation names the execution, the term
//!   orders leadership; the two are independent ([SEAT-1] v2) — and sign the lease with it;
//! - **renew** (heartbeat): re-sign the body with a fresh expiry under the SAME execution
//!   identity and term (a new execution identity can never renew: it claims fresh, term > floor);
...
//!   — signature + certificate chain to a genesis-trusted base + the per-base execution floor +
//!   the **verified leadership-term floor** ([`SeatTermLedger`], fed only by grants that passed
//!   this very acceptance). A stale claimant is refused once a higher verified term exists,
//!   regardless of the registry ([SEAT-3]).
//!
//! Fencing is safety; wall-clock expiry gates takeover liveness only. Failover is mechanism-only
//! for this program (a standby claim at a term above the floor after expiry) — nothing here
//! precludes it, and nothing here makes it automatic (fencing-is-safe-not-seamless).
...
use daemon_vhc_proto::{
    ...
    SeatRelease, SeatReleaseBody, SeatState, SeatTermLedger, SigningKey, DEFAULT_SEAT_HEARTBEAT_MS,
...
/// Derive the leadership-term bid for a fresh claim from the slot's current state and the
/// node's own persisted verified floor, or `None` when a LIVE incumbent holds the seat (the
/// caller stands by). The bid is one above the highest floor this node can see — the registry's
/// tombstone/held term OR the locally persisted verified term, whichever is higher, so a
/// registry answering stale/low state can only lose the CAS, never trick the claimant into an
/// under-floor bid. Sparse is fine: the CAS accepts any strictly greater term. A bid past the
/// bounded ordinal ceiling fails structural validation typed at authorship.
#[must_use]
pub fn derive_bid(
    state: &SeatState,
    verified_floor: Option<u64>,
    now_ms: u64,
    skew_ms: u64,
) -> Option<u64> {
    let registry_floor = match state {
        SeatState::Unclaimed {
            last_leadership_term,
        } => *last_leadership_term,
        SeatState::Leased(lease) => {
            if lease.is_expired(now_ms, skew_ms) {
                Some(lease.body.leadership_term)
            } else {
                return None;
            }
        }
    };
    Some(match (registry_floor, verified_floor) {
        (None, None) => 0,
        (a, b) => a.unwrap_or(0).max(b.unwrap_or(0)) + 1,
    })
}

/// Author a seat lease at `(incarnation, term)`: provision the coordinator per-run identity
/// (mint key + issue the certificate under the base identity) at the **counter-minted**
/// execution incarnation, and sign the lease with it. The term orders leadership; the
/// incarnation names the execution — never derived from each other ([SEAT-1] v2).
///
/// # Errors
/// Identity provisioning, keystore, or signing failure; a structurally invalid body (e.g. an
/// out-of-domain ordinal) is a typed refusal.
pub fn author_claim(
...
/// Re-sign the held lease with a fresh expiry under the SAME execution identity and term — the
/// heartbeat renew (never a takeover; the epoch/module/endpoint may change, [SEAT-2]).
///
/// # Errors
...
/// Author a release for the held lease (the registry tombstones the term; the floor persists).
///
/// # Errors
/// Keystore or signing failure.
...
/// chain to a genesis-trusted base + expiry, the **per-base execution floor** (explicit
/// revocations + the claimant's own incarnation ladder), AND the **verified leadership-term
/// floor** — a lease below the highest verified term is dead even if everything else verifies
/// ([SEAT-3] v2). The term ledger is fed ONLY by grants that passed this very acceptance
/// (never by generic certificates and never by naked registry metadata), which is what retires
/// the cross-base `role_floor()` heuristic: a coordinator-role certificate that never won the
/// seat can no longer fence the incumbent.
///
/// Returns the authorized lease's endpoint + claimant on success.
///
/// # Errors
...
pub fn authorize_incumbent(
    ...
    terms: &SeatTermLedger,
    now_ms: u64,
    skew_ms: u64,
) -> Result<AuthorizedSeat, SeatLeaseError> {
    lease.authorize(trusted_bases, now_ms, skew_ms)?;
    // The per-base execution judgment: explicit revocations + the claimant's OWN incarnation
    // ladder (a sibling's ladder never judges this sender, [CI-5] per-base).
    ...
    if revocations
        ...
        .is_err()
    {
        return Err(SeatLeaseError::Cert(daemon_vhc_proto::CertError::Revoked));
    }
    // The cross-base leadership judgment: the verified term floor.
    terms.judge(lease)?;
    Ok(AuthorizedSeat {
        ...
        leadership_term: lease.body.leadership_term,
    })
}
...
pub struct AuthorizedSeat {
    ...
    /// The claimant's execution incarnation (its per-base ladder position).
    pub incarnation: u64,
    /// The verified leadership term (the trainer's term floor for the coordinator slot).

...
/// The module's own admission for this config, through the REAL admission funnel — the source of
/// the number the production join engine enforces as the sandbox's linear-memory cap.
///
...
/// which is why this helper assembles a resource authority from the resource crate's fixture
/// assemblers instead of reading a declared claim that no longer exists.
fn admitted(wasm: &[u8], cfg_bytes: &[u8]) -> Admission {
    ...
    let report = test_support::capability_report(BackendClass::Cpu);
    ...
    let authority = ResourceAuthority {
        ...
        reservation_identity: ReservationIdentity {
            role: "trainer".into(),
            incarnation: 1,
            device_identity: "live-staging-device".into(),
            sequence: 1,
        },
        frozen_binding: None,
    };
    ...
    .expect("the trainer's own composition admits under the production trainer lane")
}

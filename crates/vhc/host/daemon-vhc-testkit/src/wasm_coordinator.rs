...
//! major-2 event-loop driver, and route frames to/from it — the harness seat that supersedes
//! [`crate::NativeCoordinator`] for the D2 matrix cells.
//!
...
/// deliver, per-sender dense seqs) exactly as [`crate::barrier`] does for workers.
...
impl WasmCoordinator {
    ...
    #[must_use]
    pub fn pump(&self) -> daemon_vhc_host::v2::PumpHandle {
        self.pump.clone()
    }
...
pub(crate) fn frame_sender(frame: &Value) -> Result<[u8; 32], String> {

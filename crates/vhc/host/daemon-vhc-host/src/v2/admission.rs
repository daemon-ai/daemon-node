        ...
        Self {
            ...
            ceilings: daemon_vhc_proto::LaneCeilings {
                ...
                worlds: ["vhc", "net", "sys", "data", "compute", "tabi"]
                    ...
                    .map(|s| (*s).to_string())
                    .collect(),
                // No host custom ops are registered pre-Phase-C; a role requesting one is
                // refused (C2's registry supplies this set when it lands).
                custom_ops: std::collections::BTreeSet::new(),
            },
        }
    }
}
...
pub fn admit_v2(
    ...
    // design; C2's custom-op gate joins this ladder after manifest decode):

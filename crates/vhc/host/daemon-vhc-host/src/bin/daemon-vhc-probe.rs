...
// The probe reads platform device memory via the same `daemon_vhc_host::autotune` FFI the worker's
// `Command::Probe` path runs; no unsafe in this bin (the FFI lives in the lib behind safe wrappers).
#![forbid(unsafe_code)]

//! `daemon-vhc-probe` — a minimal, telemetry-free device-limits readout for fleet validation (C2).
//!
//! Prints the per-platform [`daemon_vhc_host::autotune::DeviceLimits`] the worker's `Probe`/assess path
...
fn main() {
    ...
    #[cfg(windows)]
    {
        match daemon_vhc_host::autotune::probe_windows_device_limits() {
    ...
    #[cfg(target_os = "macos")]
    {
        match daemon_vhc_host::autotune::probe_macos_device_limits() {
    ...
    #[cfg(all(not(windows), not(target_os = "macos")))]
    {
        println!(
            "linux/other: the worker sources DeviceLimits from amdgpu sysfs + wgpu (feature `wgpu`); \
             run the real `daemon-vhc-worker` with DAEMON_TRAIN_PROBE=1 for the sysfs path."
        );
    }

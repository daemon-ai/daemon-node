...
// The probe reads platform device memory via the same `daemon_vhc_host::probe` FFI the worker's
...
//!
//! Prints the per-platform [`daemon_vhc_host::probe::DeviceLimits`] the worker's `Probe`/assess path
...
fn main() {
    ...
    #[cfg(windows)]
    {
        match daemon_vhc_host::probe::probe_windows_device_limits() {
    ...
    #[cfg(target_os = "macos")]
    {
        match daemon_vhc_host::probe::probe_macos_device_limits() {

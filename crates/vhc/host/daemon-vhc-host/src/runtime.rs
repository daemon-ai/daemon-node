...
//! The wasmtime host runtime (ABI §2): engine profile, host state, and the `tabi@1` dispatch layer.
//!
//! The [`Worker`] owns the wasmtime [`Engine`] (fuel on, epoch on, NaN canonicalization, pooling
//! allocator, no WASI). [`HostState`] is the frozen 66-import `tabi@1` op state
//! (params/persistents/arenas/backend) the §2.5 **compute bridge** dispatches over for major-2
//! modules ([`link_tabi`] generic over [`TabiHost`]; the v2 driver's `V2Host` embeds a
//! `HostState` and supplies the §2.5 temporal-legality rules), dispatching math to the
//! [`OpBackend`] ([`CpuBackend`] / the Burn backends), enforcing lane/handle rules
//! ([`crate::handle`]) and the memory/handle budgets.
//!
//! **The v1 five-phase lifecycle driver retired at the Phase-E sunset** (decisions D5): the
//! `Instance` `da_build → step/inner_update/make_update/ingest` dispatch, its linker, the
//! phase-legality table, and the autotune admission were removed together; a major-1 module now
//! meets a typed `AbiUnsupportedMajor` admission refusal ([`crate::select`], pinned by the
//! flipped A0 fixture). The op vocabulary and dispatch bodies below are NOT v1 leftovers — they
//! are the live bridge surface (ABI §2.5/§2.7).
...
struct ParamReg {
    #[allow(dead_code)]
    // the registered descriptor (v1 manifest surface; kept for bridge debugging)
    name: String,
    shape: Vec<u32>,
    #[allow(dead_code)]
    dtype: u32,
        ...
        match handle::classify(h) {
            Some(HandleClass::Param) => {
                let p = self
                    ...
                    .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, import, None, "param"))?;
            ...
            Some(HandleClass::Persistent) => {
                let s = self
                    .persistents
                    .get((handle::stable_index(h) - 1) as usize)
                    .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, import, None, "persist"))?;
            ...
            Some(HandleClass::Step(Lane::Native)) => self
                ...
                .resolve(h)
                .map(|(t, s)| (t, s.to_vec()))
                .map_err(|c| Trap::new(c, import, None, "native step")),
            Some(_) => Err(Trap::new(
        ...
        match handle::classify(h) {
            Some(HandleClass::DetPersistent) => {
                let s = self
                    ...
                    .get((handle::stable_index(h) - 1) as usize)
                    .ok_or_else(|| {
                        Trap::new(TrapCode::InvalidHandle, import, None, "detpersist")
                    })?;
                Ok((s.tensor, s.shape.clone()))
            }
            Some(HandleClass::Step(Lane::Det)) => self
                ...
                .resolve(h)
                .map(|(t, s)| (t, s.to_vec()))
                .map_err(|c| Trap::new(c, import, None, "det step")),
            Some(_) => Err(Trap::new(
                TrapCode::LaneMismatch,
                import,
                None,
...
impl Worker {
    ...
    /// module imports an op outside the host vocabulary before ever instantiating it.
    ///
    /// # Errors
    ///
...
impl HostState {
    ...
    fn register_param(
        ...
        if dtype > 7 {
            return Err(Trap::new(TrapCode::BadEnum, "param@1", None, "dtype"));
        }
        if init > 4 {
            return Err(Trap::new(TrapCode::BadEnum, "param@1", None, "init"));
        }
        ...
        match handle::classify(dst) {
            Some(HandleClass::Param) => {
                ...
                let p = self
                    .params
                    .get(idx)
                    .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, "assign@1", None, "param"))?;
            ...
            Some(HandleClass::Persistent) => {
                ...
                let t = self
                    .persistents
                    .get(idx)
                    .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, "assign@1", None, "persist"))?
                    .tensor;
    ...
    fn batch_ref(&self, import: &'static str, b: u64) -> Result<&BatchData, Trap> {
        match handle::classify(b) {
            Some(HandleClass::Batch) => self
                ...
                .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, import, None, "batch")),
            _ => Err(Trap::new(
                TrapCode::InvalidHandle,
                import,
                None,
    ...
    fn staged_section(&self, import: &'static str, i: u32, s: u32) -> Result<&Section, Trap> {
        let ci = *self
            .staged
            .get(i as usize)
            .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, import, None, "staged index"))?;
        self.containers[ci]
            .sections
            .get(s as usize)
            .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, import, None, "section index"))
    }
    ...
    fn op_upd_sections(&self, i: u32) -> Result<u32, Trap> {
        let ci = *self
            .staged
            .get(i as usize)
            .ok_or_else(|| Trap::new(TrapCode::InvalidHandle, "upd_sections@1", None, "staged"))?;
    ...
    fn op_det_sum(&mut self, handles: &[u64]) -> Result<u64, Trap> {
        ...
        let out = self
            .backend
            .det_sum(&ids)
            .map_err(|c| Trap::new(c, "det_sum@1", None, "shapes"))?;
    ...
    fn op_det_binary(&mut self, import: &'static str, a: u64, b: u64) -> Result<u64, Trap> {
        ...
        .map_err(|c| Trap::new(c, import, None, "shapes"))?;
    ...
    fn op_det_absmax_unpack(&mut self, packed: u64, chunk: u32, bits: u32) -> Result<u64, Trap> {
        ...
        let out = self
            ...
            .map_err(|c| Trap::new(c, "det_absmax_unpack@1", None, "layout"))?;
    ...
    fn op_det_chunk_scatter_add(
        ...
        self.backend
            ...
            .map_err(|c| Trap::new(c, "det_chunk_scatter_add@1", None, "layout"))
    }
    ...
    fn op_det_chunk_scatter(
        ...
        let out = self
            ...
            .map_err(|c| Trap::new(c, "det_chunk_scatter@1", None, "layout"))?;
        ...
        match handle::classify(dst) {
            Some(HandleClass::DetPersistent) => {
                let t = self
                    ...
                    .ok_or_else(|| {
                        Trap::new(TrapCode::InvalidHandle, "det_assign@1", None, "detpersist")
                    })?
                    .tensor;
                self.backend.write(t, &data);
                Ok(())
    ...
    fn op_det_axpy_param(&mut self, p: u64, x: u64, alpha: f64) -> Result<(), Trap> {
        ...
        self.backend
            ...
            .map_err(|c| Trap::new(c, "det_axpy_param@1", None, "shapes"))?;
        let m_v = self.backend.view(master).to_vec();
        self.backend.write(storage, &m_v);
        Ok(())
    }
    ...
    fn op_rmsnorm(&mut self, x: u64, w: u64, eps: f64) -> Result<u64, Trap> {
        ...
        let d = *xsh
            .last()
            .ok_or_else(|| Trap::new(TrapCode::ShapeMismatch, "rmsnorm@1", None, "rank"))?
            as usize;
    ...
    fn op_softmax(&mut self, x: u64, dim: u32) -> Result<u64, Trap> {
        ...
        if dim >= xsh.len() {
            return Err(Trap::new(TrapCode::ShapeMismatch, "softmax@1", None, "dim"));
        }
    ...
    fn op_rope(
        ...
        if xsh.len() < 2 {
            return Err(Trap::new(TrapCode::ShapeMismatch, "rope@1", None, "rank"));
        }
    ...
    fn op_slice(&mut self, x: u64, dim: u32, start: u32, end: u32) -> Result<u64, Trap> {
        ...
        if dim >= xsh.len() || start > end || end > xsh[dim] as usize {
            return Err(Trap::new(TrapCode::ShapeMismatch, "slice@1", None, "range"));
        }
        let shape_in: Vec<usize> = xsh.iter().map(|&d| d as usize).collect();
    ...
    fn op_topk_chunk(&mut self, x: u64, chunk: u32, k: u32) -> Result<(u64, u64), Trap> {
        ...
        let (vt, it) = self
            ...
            .map_err(|c| Trap::new(c, "topk_chunk@1", None, "layout"))?;
    ...
    fn op_chunk_scatter(
        ...
        let out = self
            .backend
            .det_chunk_scatter(valst, idxt, chunk as usize, Self::numel(dims))
            .map_err(|c| Trap::new(c, "chunk_scatter@1", None, "layout"))?;
        self.alloc_native(out, dims.to_vec())
    }
    ...
    fn op_absmax_pack(&mut self, x: u64, chunk: u32, bits: u32) -> Result<u64, Trap> {
        ...
        let out = self
            ...
            .map_err(|c| Trap::new(c, "absmax_pack@1", None, "layout"))?;
        let n = self.backend.view(out).len() as u32;
    ...
    fn op_absmax_unpack(&mut self, packed: u64, chunk: u32, bits: u32) -> Result<u64, Trap> {
        ...
        let out = self
            .backend
            .det_absmax_unpack(pt, chunk as usize, bits)
            .map_err(|c| Trap::new(c, "absmax_unpack@1", None, "layout"))?;
        let n = self.backend.view(out).len() as u32;
        self.alloc_native(out, vec![n])
    }
    ...
    fn op_dct2(&mut self, x: u64, tile: u32, import: &'static str) -> Result<u64, Trap> {
        ...
        let out = self
            ...
            .map_err(|c| Trap::new(c, import, None, "tile"))?;
        self.alloc_native(out, xsh)
    }
    ...
    fn op_idct2_native(&mut self, x: u64, tile: u32) -> Result<u64, Trap> {
        ...
        let out = self
            ...
            .map_err(|c| Trap::new(c, "idct2@1", None, "tile"))?;
        self.alloc_native(out, xsh)
    }
    ...
    fn op_det_idct2(&mut self, x: u64, tile: u32) -> Result<u64, Trap> {
        ...
        let out = self
            .backend
            .idct2(xt, tile as usize)
            .map_err(|c| Trap::new(c, "det_idct2@1", None, "tile"))?;
        self.alloc_det(out, xsh)
    }
        ...
        match handle::classify(h) {
            Some(HandleClass::Step(Lane::Native)) => {
                let t = self
                    .step_native
                    .free(h)
                    .map_err(|c| Trap::new(c, "drop@1", None, "native step"))?;
                self.backend.free(t);
                Ok(())
            }
            Some(HandleClass::Step(Lane::Det)) => {
                let t = self
                    .step_det
                    .free(h)
                    .map_err(|c| Trap::new(c, "drop@1", None, "det step"))?;
                self.backend.free(t);
                Ok(())
            }

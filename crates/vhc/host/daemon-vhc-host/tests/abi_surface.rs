...
// agree name-for-name with the frozen `tabi@1` import list (`daemon_vhc_abi::TABI_IMPORTS`).
// The vocabulary lives in the dual-compiled `daemon-vhc-abi` contract crate, so this assertion no
// longer dev-depends on the guest SDK. Extended from Wave 1's phase-table coverage: the vocabulary
// grows additively (Merge-1 50 + the Wave-2 additions), and this test is the tripwire that a new op
// landed on BOTH sides or neither.

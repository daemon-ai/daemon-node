...
- **Dispatch ownership — subscriber model (v1), now backed by reusable infrastructure.** Delivery is a
  *subscriber set* (§5.4): the transport instance named in the Primary discovers its sessions and posts.
  ...
  `WireVersion` v9) is the owned-session discovery primitive a transport calls on (re)connect — an
...
10. **Bidirectional routing is a host capability (§5.9).** A host-level routing registry maps
    ...
    `delivery_sessions` owned-session discovery (wire v9) + the `daemon-delivery` pull subscriber for
...
- **P4.5 — bidirectional routing (§5.9).** The host routing registry + profile-aware
  ...
  *Outbound half landed:* `delivery_sessions` discovery (wire v9), the in-process `DeliverySink` push

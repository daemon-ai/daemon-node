...
Revision 20 (2026-08-11) closes the defaults-freeze evidence pass, offline from the banked
C2 artifacts: RQ-4 RESOLVED (floor 10 min + 2× multiplier frozen against the measured C2
cadence — healthy 5–8 min, checkpoint-follow 13.6–20.9 min, three simulated trips all
justified or benign-one-shot), RQ-9 RESOLVED (effective growth ~1.9 GiB/round measured;
the gate fed that figure would have refused C2's 60 GiB authoring; 25% headroom frozen),
and RQ-5's round-44/46 storage suspicion DISCONFIRMED (no storage evidence in those
windows; the non-heal is transport-side, residual unchanged). Register + §9 verdict note.
...
*Evidence pass verdict (2026-08-11): the storage chain holds for the Incident-2/suspension
windows (journal-proven, §10) but does NOT hold for round 44/46 — no storage trap or
refusal exists in those windows (all three `HostStorageExhausted` records are rounds
~30–33; the post-resume wiped scopes were clean), and the round-46 telemetry shows
`failed_retryable retries=1`: the keeper retried into the SAME standing gap and parked,
neither budget-exhausted nor storage-gated. The non-heal there is the transport story
after all — consistent with the deaf-window loss boundary below (ring evicted ⇒ only a
fresh incarnation heals) — and its plane-level cause stays the RQ-5 residual (endpoint
logs were not preserved).*
...
| RQ-4 | **RESOLVED — defaults frozen (2026-08-11, offline evidence pass over the banked C2 transcript, rounds 0–32 at 30 s cadence; the collector stopped 08-10 23:03Z so rounds 33–47 rest on the ledger).** Healthy steady-state inter-commit gap 5–8 min; checkpoint-follow rounds (slots at 0/4/8) stretch to 13.6–20.9 min — the checkpoint-heavy validation the freeze waited for. Replaying `max(10 min, 2 × slowest observed)` over the observed sequence trips exactly three times: r02 (20.9 m vs 14.6 m — the FIRST checkpoint-stretched round, before the adaptive half has seen one; a benign one-shot warning that self-clears on commit, the first-round question answered as acceptable), r17 (50.1 m vs 44.8 m — genuine: the 2-trainer 3600 s-wall degradation), r20 (268.7 m vs 118.2 m — the genuine overnight stall, announced ~2 h in where the operator took 4.2 h). Floor 10 min + multiplier 2× FROZEN; an authored round wall stays future vocabulary | frozen; revisit only on false-positive evidence from a run with a different round-wall shape |
| RQ-5 | **Partially resolved (§2.1/§9c); the round-44/46 storage suspicion DISCONFIRMED (2026-08-11 evidence pass).** The non-heal mechanism was the unbounded co-trainer respawn cycle, pinned. The §9 suspicion that the storage seam blocked the keeper in the round-44/46 windows does NOT hold on banked evidence: all three `HostStorageExhausted` traps in the rung-0 inventory are suspension-era (rounds ~30–33, post-wipe scopes were clean), and the round-46 telemetry (`failed_retryable retries=1`) shows a keeper neither budget-exhausted nor storage-gated — its retry landed in the SAME standing gap and only a fresh operator cycle healed it, exactly the deaf-window loss boundary §9's lattice pins (rebroadcast ring evicted ⇒ only rejoin heals). RESIDUAL unchanged: the underlying iroh-vs-WS packet loss behind the standing gap (endpoint logs were not preserved) | §9(a) gap-hold visibility + preserved endpoint logs on the next ceremony |
...
| RQ-9 | **RESOLVED — growth figure sourced and margin frozen (2026-08-11, offline evidence pass over the banked C2 ledger + rung-0 inventory).** Measured: all three 60 GiB scopes exhausted by round ~33 of 48 (journal-proven: three `HostStorageExhausted` traps carrying the 64424509440 B quota / free-floor figures); M4's wipe reclaimed 61.4 GiB over 33 rounds ≈ **1.9 GiB/round EFFECTIVE growth** (restore/recycle stacking included — the honest planning figure, since stacking is exactly what fills a scope under incident churn); durable checkpoint spike 12.6 GB. Fed the honest figure, the shipped gate demands ≥ 48 × 1.9 × 1.25 ≈ **114 GiB** — it would have REFUSED C2's 60 GiB authoring, which is the gate doing its job. The 25% headroom (~28 GiB at that scale) covers the 12.6 GB checkpoint spike with margin — FROZEN. Authoring guidance: source growth from the banked effective figure (~2 GiB/round for tiny-llama-class runs) until a fit-probe automates it; REL-8's reconcile shrinks the stacking term at the source but the gate keeps pricing it | frozen; fit-probe automation stays future work (§10 deviation note) |

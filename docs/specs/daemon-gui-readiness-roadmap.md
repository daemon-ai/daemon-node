...
| Prompt assembly + Anthropic prompt caching | `agent/system_prompt.py:387`; `agent/prompt_caching.py:49` | **DONE** — the system prompt is a `ComposedPrompt` of typed ordered `SlotKind` slots (Identity/Guidance/ContextFiles/SkillsIndex/MemoryBlock/UserProfile/Stamp), composed once per session and byte-stable across turns; per-turn recall/nudges ride an ephemeral `TurnInjection` on the outgoing request's last user message, never the prefix ([context.rs](../../crates/engine/daemon-core/src/context.rs)). Personas are node-owned SOUL.md read/edited via `SoulGet`/`SoulSet` (wire v36) — never a client-settable spec field; the prompt *content* + stores live in [crates/prompt/daemon-prompt](../../crates/prompt/daemon-prompt). `mark_cache_breakpoints` applies the `system_and_3` policy driving genai `cache_control` ephemeral blocks (tools+system + last 3 messages, ≤4 total, 5m/1h TTL, wire-tested) + an OpenAI `prompt_cache_key`, and mistral.rs (`prefix_cache_n=16`) + the llama.cpp persistent KV-cache reuse prefixes ([provider.rs](../../crates/engine/daemon-core/src/provider.rs), [genai_provider.rs](../../crates/providers/daemon-providers/src/genai_provider.rs)). P1. |

...
impl ComputeServer for CudaServer {
    ...
    fn initialize_memory(&mut self, memory: ManagedMemoryHandle, size: u64, stream_id: StreamId) {
        ...
        // A fallible device/pool reservation records the failure on the stream's error queue
        // instead of unwrapping into a panic, so a subsequent `sync`/`flush` (fence) can drain it
        // as a typed `ServerError` — mirroring the `launch` and `write` paths, which push their
        // failures via `command.error`. Without this, the caught-and-dropped panic left no error
        // state for a fence to observe: allocation faults were readback-visible only. This covers
        // both the host-side pool-cap rejection (`IoError::BufferTooBig` before any `cuMemAlloc`)
        // and a genuine driver `CUDA_ERROR_OUT_OF_MEMORY` (also surfaced as `IoError` by the GPU
        // storage layer), so either becomes fence-visible.
